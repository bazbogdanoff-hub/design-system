...
// fontSize and fontWeight survive variable binding cleanly — binding lineHeight
// or letterSpacing force-converts the field to PIXELS in Figma, so those two
// are NOT made into variables; they live as raw PERCENT values on each style.
//   fontSize   -> px          fontWeight -> 500/600/700/800
...
for (const [p, raw] of Object.entries(rawByPath)) {
  ...
  } else if (p.match(/^font\.weight\.\w+$/)) {
    floats.push({ name: figName(p), value: Number(raw) });
  } else if (p === 'font.family.sans') {
    strings.push({ name: 'font/family/sans', value: 'Plus Jakarta Sans' });
  } else if (p === 'font.family.mono') {
    strings.push({ name: 'font/family/mono', value: 'Roboto Mono' });
  }
}
...
const compAliases = [];
for (const [p, raw] of Object.entries(rawByPath)) {
  if (!isColorComponent(p) && !/^radius\.(card|modal|table|popover|button|input|badge)(\.(sm|md|lg|xl))?$/.test(p)) continue;
  const m = typeof raw === 'string' && raw.match(/^\{(.+)\}$/);
...
(function walkType(node, path) {
  if (node && node.$value && typeof node.$value === 'object' && 'fontSize' in node.$value) {
    ...
    const refKey = (ref, group) => { const m = String(ref).match(new RegExp(`font\\.${group}\\.(\\w+)`)); return m ? m[1] : null; };
    // only size / weight / family are bindable (see 01 comment)
    textStyles.push({
      name: 'text/' + path.join('/'),
      ...
      // token keys, for binding each field to its font/* primitive variable
      sizeKey: refKey(v.fontSize, 'size'),
...
for (const s of STRINGS) {
  let v = byName.get(s.name);
  if (!v) { v = figma.variables.createVariable(s.name, coll, 'STRING'); created++; } else updated++;
  v.hiddenFromPublishing = true;
...
for (const s of SPEC) {
  ...
  // those fields to PIXELS, which destroys a ratio line-height and an em
  // tracking. They stay as raw PERCENT style values (unbind here in case an
  // earlier run bound them).
  const bind = (field, vname) => {
    if (!vname) return;
    const v = varByName.get(vname);
    if (!v) { missingVars.add(vname); return; }
    try { st.setBoundVariable(field, v); } catch (e) { missingVars.add(vname + ' (' + field + ': ' + e.message + ')'); }
  };
  bind('fontFamily', s.mono ? 'font/family/mono' : 'font/family/sans');
  bind('fontSize', s.sizeKey && ('font/size/' + s.sizeKey));
  bind('fontWeight', s.weightKey && ('font/weight/' + s.weightKey));
  try { st.setBoundVariable('lineHeight', null); } catch (e) {}
  try { st.setBoundVariable('letterSpacing', null); } catch (e) {}
  st.lineHeight = { unit: 'PERCENT', value: s.lineHeightPct };
  st.letterSpacing = { unit: 'PERCENT', value: s.letterSpacingPct };
}
